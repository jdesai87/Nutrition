...
import { MacroTotals, MealType, UserSettings } from "@/lib/types";
...
export default function MealSuggestions({ totals, settings }: MealSuggestionsProps) {
  ...
  const { suggestions, message } = getMealSuggestions(totals, settings, nextMeal);
        ...
        <div className="space-y-2">
          {suggestions.map((s, i) => (
            <div key={i} className="px-3 py-2.5 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800 rounded-lg">
              <p className="text-sm font-medium text-emerald-900 dark:text-emerald-100">{s.food}</p>
